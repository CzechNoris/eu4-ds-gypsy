...
for file_path in file_paths:
    ...
    # Remove keys: ai_will_do, potential
        ...
        # - there are at least 3 keys in the object - additional keys for effects
        ...
        for k, v in obj.items():
            if 'monarch_power' not in v:
                ...
                if v['monarch_power'] not in POLICY_SLOT_VALUES:
                    logging.error(
                        f'{k}: wrong monarch_power value: {v["monarch_power"]}')
            if 'allow' not in v:
        ...
        # Parse allow section
        rel_idea_names = ['religious_ideas', 'anglican0', 'animist0', 'buddhism0', 'cathar0', 'catholic0', 'confucian0', 'coptic0', 'dreamtime0', 'fetishist0', 'hellenic0', 'hindu0', 'hussite0', 'ibadi0', 'inti0',
        ...
        gov_idea_names = ['dictatorship0', 'horde0',
                        'monarchy0', 'republic0', 'theocracy0']
        for k, v in obj.items():
            allow_keys = v['allow'].keys()
            if len(allow_keys) == 1:
                # Simple case with two full_idea_group
                if 'full_idea_group' in allow_keys:
                    ...
                    if isinstance(full_idea_group, list) and len(full_idea_group) == 2:
                        ...
                        logging.warning(
                            f'{k}: unknown allow structure: {v["allow"]}')
                # Case with 2 OR groups 
                    ...
                    if isinstance(OR_group, list) and len(OR_group) == 2 and 'full_idea_group' in OR_group[0] and 'full_idea_group' in OR_group[1]:
                        v['req'] = [OR_group[0]['full_idea_group'],
                                    OR_group[1]['full_idea_group']]
                    else:
                        logging.warning(
                            f'{k}: unknown allow structure: {v["allow"]}')
                else:
                    logging.warning(f'{k}: unknown allow structure: {v["allow"]}')
            # Case of a combination of full_idea_group, an OR group, has_completed_religious_ideas_group and has_completed_government_ideas_group
        ...
        # - every object has to have keys req which is a list of 2 lists
        for k, v in obj.items():
            if 'allow' in v:
                del v['allow']
            if 'req' not in v:
                ...
                if not isinstance(v['req'], list):
                    logging.warning(f'{k}: req is not a tuple')
                else:
                    if len(v['req']) != 2:
                        logging.warning(
                            f'{k}: req tuple has wrong length: {len(v["req"])}')
                    else:
        ...
        logging.error(f'Exception while processing {file_path}: {e}')
